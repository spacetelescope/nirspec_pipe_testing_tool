...
def pathtest(step_input_filename, reffile, comparison_filename,
    ...
    for slit, slit_num in zip(pl_ifu_slits, slit_list):
        ...
        if show_figs:
            plt.show()
        if save_figs:
            plt_name = step_input_filename.split('_flat_field')[0] + "_Pathloss_test_IFU_UNI_slit_" + str(slit_num) + ".png"
    ...
    if FINAL_TEST_RESULT:
        msg = " *** Final result of pathloss test is PASSED *** "
        print(msg+"\n")
        log_msgs.append(msg)
        result_msg = "All slits PASSED path_loss test."
    else:
        msg = " *** Final result of pathloss test is FAILED *** "
        print(msg+"\n")
        log_msgs.append(msg)
