...
Example usage:
    ...
    To simply see the header on-screen type:
        $ nptt_read_hdr blah.fits
    To see and save the header into a text file, add the flag -s to the end of the previous command.
    
    To see and save the header of a different extension from the main, use the flag -e=extension_number:
        $ nptt_read_hdr blah.fits -s -e=1
...
def read_hdr(fits_file_name, save_txt=False, ext_number=None):
...
def main():
    ...
    read_hdr(fits_file_name, save_txt, ext_number)
